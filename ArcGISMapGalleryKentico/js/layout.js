...
function initFeaturedMapsAndApps() {
    var groupID = GalleryGroupId; //document.getElementById("<%=GroupID.ClientID%>').value; //"93ddb89b6d0d4afe9d9d9ebdb2a933cf";
    ...
    esri.request({
        ...
        load: function (itemsResponse) {
            var widthStyleValue = galleryWidth + "px";
            dojo.query("#featuredMaps").style("width", widthStyleValue);
...
function addToPane(results, paneNodeId) {
    ...
    dojo.forEach(results, function (resultItem) {
        ...
        // THUMBNAIL
        var thumbUrl = (resultItem.thumbnail) ? window.esriGeowConfig.restBaseUrl + dojo.replace("content/items/{id}/info/{thumbnail}", resultItem) : 'CMSWebParts/ArcGISMapGalleryKentico/images/defaultThumb.png';
        // ITEM URL
